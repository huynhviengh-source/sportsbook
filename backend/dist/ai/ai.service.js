...
let AiService = class AiService {
    async chat(userMessage, courts) {
        ...
        if (msg.includes('pickleball')) {
            ...
            if (filtered.length === 0)
                return 'Hiện tại chưa có sân Pickleball. Vui lòng thử lại sau!';
        ...
        if (msg.includes('cầu lông') || msg.includes('badminton') || msg.includes('cau long')) {
            ...
            if (filtered.length === 0)
                return 'Hiện tại chưa có sân cầu lông. Vui lòng thử lại sau!';
            const list = filtered.map(c => `${c.name} (${c.address}) - ${c.price_per_hour.toLocaleString('vi-VN')}đ/giờ`).join('\n');
        ...
        if (msg.includes('tennis')) {
            ...
            if (filtered.length === 0)
                return 'Hiện tại chưa có sân Tennis. Vui lòng thử lại sau!';
            const list = filtered.map(c => `${c.name} (${c.address}) - ${c.price_per_hour.toLocaleString('vi-VN')}đ/giờ`).join('\n');
        ...
        if (msg.includes('bóng đá') || msg.includes('bong da') || msg.includes('football')) {
            ...
            if (filtered.length === 0)
                return 'Hiện tại chưa có sân bóng đá. Vui lòng thử lại sau!';
            const list = filtered.map(c => `${c.name} (${c.address}) - ${c.price_per_hour.toLocaleString('vi-VN')}đ/giờ`).join('\n');
    ...
    async suggestCourts(requirement, courts) {
        const req = requirement.toLowerCase();
        let filtered = courts;
        if (req.includes('pickleball'))
            filtered = courts.filter(c => c.sport_type === 'pickleball');
        else if (req.includes('cầu lông') || req.includes('badminton'))
            filtered = courts.filter(c => c.sport_type === 'badminton');
        else if (req.includes('tennis'))
            filtered = courts.filter(c => c.sport_type === 'tennis');
        else if (req.includes('bóng đá') || req.includes('football'))
            filtered = courts.filter(c => c.sport_type === 'football');
    ...
    async generateReviewReply(reviewComment, rating, courtName) {
        if (rating >= 4) {
